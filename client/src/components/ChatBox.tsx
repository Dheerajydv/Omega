...
const ChatBox = ({ handleChatBack }: ChatBoxProps) => {
    ...
    return (
        <>
            <div onClick={handleChatBack} className="navbar sticky z-10 top-0 bg-primary text-primary-content">
                <button className="btn btn-ghost text-xl">⬅️</button>
                <div className="avatar flex justify-center items-center">
                    <div className="ring-primary mr-2 ring-offset-base-100 w-10 rounded-full ring-2 ring-offset-2">
                        <img className="h-10" src={selectedChat.profilePic ? selectedChat.profilePic : "../../public/user.png"} alt="Profile Picture" />
                    </div>
                        ...
                        : messages?.length > 0 && messages?.map((message) => (
                            <div className='' key={message?._id} ref={lastMessageRef}>
                                <div className={`chat ${message.senderId === authUser.data._id ? 'chat-end' : 'chat-start'}`}>
                                    <div className={`avatar ${message.image ? "" : "hidden"}`}>
                                        <div className="w-24 rounded">
                                            <a target="main" href={message?.image}><img src={message?.image} /></a>
                                        </div>
                                    </div>
                                    <div className={`chat-bubble ${message.senderId === authUser.data._id ? 'bg-primary' : 'bg-secondary'} ${message.image ? "hidden" : ""}`}>
                                        {message?.message}
                                    </div>
